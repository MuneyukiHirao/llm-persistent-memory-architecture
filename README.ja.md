...
### 6. 専門エージェントの制約：1タスク1睡眠

専門エージェントは中間睡眠を持たない。1タスク = 1セッション = 1睡眠。
...
├── 中間睡眠なし
├── コンパクション = タスク失敗
└── オーケストレーターが適切なサイズにタスク分割する責任
```

### 7. 進捗監視と早期中断

オーケストレーターはエージェントの進捗を監視し、コンパクション前に介入。
...
判断:
├── 順調 → 継続
├── 危険（進捗 < コンテキスト使用率）→ 早期中断を指示
└── 行き詰まり → ユーザーに報告
```

### 8. 進捗状態の外部記録

オーケストレーターはタスク指示/結果受領時に進捗を外部ファイルに記録。

- ユーザーがいつでも進捗を確認できる
- オーケストレーターの中間睡眠後も復帰可能
- タスク依存関係ツリー（task_tree）を管理
- 成果物のファイルパスを記録し、後続タスクに引き継ぎ
...
│  睡眠フェーズ（タスク完了ごと）                              │
├─────────────────────────────────────────────────────────────┤
│  タスク完了 → 学び抽出 → 減衰処理 → アーカイブ → 睡眠       │
└─────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────┐
│  進捗状態ファイル（外部記録）                                │
├─────────────────────────────────────────────────────────────┤
│  タスクツリー、依存関係、成果物パス、問題・決定履歴         │
│  → ユーザーが進捗確認可能、中間睡眠からの復帰に使用         │
└─────────────────────────────────────────────────────────────┘
```
