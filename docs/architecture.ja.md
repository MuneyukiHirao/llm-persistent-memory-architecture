...
オーケストレーターは単なるルーティング担当ではなく、**評価と学習のハブ**である。

#### 役割の拡張

```
従来の理解：
オーケストレーター = タスクを適切なエージェントに振り分けるだけ
...
| メモリ種別 | 内容 | 管理方法 |
|-----------|------|---------|
| ユーザープロファイル | 表現の癖、好み、反応パターン | DLによる強化学習 |
| エージェント専門性 | 誰がどのタスクで成功するか | タスク結果の統計 + ユーザー評価 |
| ルーティングモデル | どの状況で誰に振るべきか | 上記2つを入力とした学習 |
| 会話コンテキスト | 現在の会話の流れ | セッション内で保持、セッション終了時にクリア |
| 担当履歴 | 誰にどのタスクを振ったか | 時間経過で減衰 |
...
| 大幅に修正して使用 | 部分失敗 | -0.3 |
| やり直しを依頼 | 失敗 | -1.0 |
| 別のエージェントに再依頼 | ルーティング誤り | エージェントスコア変更なし、ルーティングモデル更新 |
...
class UserProfile:
    ...
    # 表現の好み
    preferred_formality: float      # フォーマル ↔ カジュアル
    preferred_detail_level: float   # 詳細 ↔ 簡潔
    preferred_explanation: float    # 説明重視 ↔ 結果重視

    # 反応パターン
    patience_level: float           # 待てる ↔ すぐ欲しい
    error_tolerance: float          # 寛容 ↔ 厳格

    # 専門性
    domain_expertise: dict          # 分野ごとの専門度
...
class AgentExpertise:
    """エージェントの専門性をオーケストレーターが管理"""

    agent_id: str

    # タスクタイプごとの成功率
    success_rate_by_task_type: dict  # {"見積もり": 0.85, "交渉": 0.72, ...}

    # ユーザー満足度（オーケストレーターが評価）
    user_satisfaction_score: float   # 0.0 - 1.0

    # 最近のパフォーマンス（減衰あり）
    recent_performance: list         # 直近N件の成功/失敗
...
オーケストレーターは専門エージェントと異なり、**会話セッション終了時に睡眠**する。
...
def orchestrator_sleep(orchestrator_memory):
    ...
    # 1. 会話コンテキストをクリア（永続化不要）
    orchestrator_memory.conversation_context = None

    # 2. 担当履歴は減衰
    for history in orchestrator_memory.assignment_history:
        history.strength *= 0.99  # 緩やかな減衰

    # 3. エージェント専門性は減衰しない（明示的更新のみ）
    # → update_from_task_result() で更新済み

    # 4. ユーザープロファイルは減衰しない（蓄積型）
    # → update_from_feedback() で更新済み

    # 5. ルーティングモデルはバッチ学習で更新
...
```python
class RoutingModel:
    """どのエージェントにルーティングすべきかを学習"""

    def predict(self, task_summary, user_profile):
        """
        入力:
        - task_summary: タスクの概要
        - user_profile: ユーザーの好みと傾向
...
4. フィードバックループ
   ユーザー反応 → オーケストレーター評価 → エージェント専門性更新
                                        → ルーティング改善
                                        → ユーザープロファイル更新
```
...
| オーケストレーター | ルーティング・評価・学習のハブ。会話コンテキストとユーザープロファイルを管理 |
| ユーザープロファイル | オーケストレーターが学習するユーザーの好みと傾向。表現の癖や反応パターンを含む |
| エージェント専門性スコア | オーケストレーターが管理する各エージェントの成績評価。ユーザーフィードバックに基づく |
| ルーティングモデル | タスクとユーザープロファイルからどのエージェントに振るべきかを予測するモデル |
