...
```json
{
  ...
  "system_prompt": "あなたはユーザーとの対話を管理し、適切なエージェントにタスクを振り分けるオーケストレーターです...",
...
{
  "id": "mem_501",
  "content": "ユーザーAは見積もりタスクでProcurement_01に依頼したが、やり直しを要求した",
  "learnings": {
    "ユーザー意図": "ユーザーAは詳細な根拠を求める傾向がある",
    "エージェント適性": "Procurement_01は概算は得意だが詳細見積もりは苦手",
    "会話の流れ": "見積もり→やり直し要求→詳細版を再依頼",
    "タスク結果評価": "この組み合わせは失敗だった",
    "ユーザー満足度": "やり直し要求 = 不満足シグナル"
  },
  "strength": 1.0,
...
オーケストレーターも専門エージェントと同じ外部メモリを持つため、過去のセッションの内容を記憶できる。

```
ユーザー：「おとといやっていた作業ってどうなった？」

オーケストレーター：
1. 「会話の流れ」観点で外部メモリを検索
2. 関連する過去のセッション記憶がヒット
3. その記憶の strength が強化される
4. 回答を生成
```

過去の会話も強度管理と減衰の対象となるため：
- よく参照される会話は定着する
- 参照されない古い会話は自然に忘れる
- 「あの時の件」と言われて思い出せる（検索でヒットすれば）

#### 評価フロー

```
1. オーケストレーター → 専門エージェントにタスク依頼
2. 専門エージェント → 結果を返す（+ 睡眠）
3. オーケストレーター → ユーザーに提示
4. ユーザー → 反応（明示的/暗黙的フィードバック）
5. オーケストレーター → 成功/失敗を判定し、学びを記憶
6. オーケストレーター → 睡眠（タスク完了として）
```

**重要な設計判断：評価はオーケストレーターが行う**

```
問題：
エージェント自身が自己評価 → 人間の期待値との乖離が修正されない
「正しく処理した」と「人間が満足した」は別

解決：
オーケストレーターが人間のフィードバックを観察して評価
→ 「エージェント適性」観点の記憶として保存
→ 次回のルーティング判断時に検索でヒット
```
...
ユーザーは常に明示的に「良かった/悪かった」と言うわけではない。以下のシグナルを検出する：
...
```
従来の設計（特別扱い）：
UserProfile クラスで明示的に管理
→ 減衰しない、構造化された属性

統一された設計：
通常の記憶として蓄積
→ 「ユーザーAは詳細を好む」という学びが記憶される
→ 強度管理と減衰が適用される
→ よく参照される傾向は定着する
→ 長期間確認されない傾向は自然に忘れる
```

これにより、ユーザーの好みも「変化しうるもの」として扱える。昔は詳細を好んでいたが、最近は簡潔を好むようになった、といった変化に自然に適応できる。

#### 設計の要点

```
1. 統一されたメカニズム
   - オーケストレーターも専門エージェントも同じ仕組み
   - 外部メモリ、強度管理、減衰、定着、容量制限
   - 違いは観点と役割だけ

2. 評価の分離
   - 専門エージェント：自分の学びを保存（ドメイン知識）
   - オーケストレーター：成功/失敗を判定（ユーザー視点）
...
オーケストレーターも専門エージェントと同じ教育プロセスで初期状態を形成する。空の外部メモリから始めると、適切な判断ができない。
...
orchestrator_textbook:
  ...
  chapters:
    - title: "曖昧な指示への対応"
      sections:
        - title: "曖昧さの検出"
          text: "ユーザー指示に複数の解釈が可能な場合、勝手に解釈せず質問で明確化する..."
        - title: "質問の仕方"
          text: "選択肢を提示する形式が効果的。オープンな質問は避ける..."
      test_questions:
        - question: "「アプリを作って」という指示を受けた。どう対応すべきか？"
          answer_keywords: ["質問", "明確化", "プラットフォーム", "機能"]

    - title: "エージェント選択"
      sections:
        - title: "専門性の見極め"
          text: "タスクの性質を分析し、適切なエージェントを選択する..."
      test_questions:
        - question: "「日本語入力の問題を調査」と「UIを実装」は同じエージェントに任せるべきか？"
          answer_keywords: ["調査", "実装", "別エージェント"]
```

**重要な初期学び**:

```
オーケストレーターが最初から持っているべき学び:
...
4. タスクサイズの判断
   - 「専門エージェントのコンテキストサイズを把握する」
   - 「マージンを持ってタスクを分割する」
   - 「コンパクション失敗は自分の指示が悪かったと学習する」

5. 進捗監視と介入
   - 「一定時間経過したら進捗確認を送信」
   - 「停滞を検出したら早期中断を指示」
   - 「行き詰まりはユーザーに報告して判断を仰ぐ」
...
これらは教育プロセスで外部メモリに蓄積され、実運用での経験で強化・減衰される。

#### タスクサイズの管理

**オーケストレーターは部下（専門エージェント）のコンテキストサイズを理解し、マージンを持ってタスク指示する責任を持つ。**

```
オーケストレーターが把握すべき情報:
├── 各エージェントのコンテキストウィンドウサイズ
├── タスクの種類ごとの典型的なコンテキスト消費量
└── 安全マージン（例: 70%以内で完了できるサイズ）
```
...
```
悪い例:
タスク: 「Android SSH/SFTPクライアントのTerminalViewを実装」
→ 大きすぎる、エージェントがコンパクションを起こす

良い例:
タスク1: 「TerminalViewの基本描画を実装」
タスク2: 「キー入力イベント処理を実装」
タスク3: 「特殊キーパネルを実装」
タスク4: 「描画とキー入力を統合」
→ 各タスクは1セッションで完了できるサイズ
```
...
def handle_agent_failure(orchestrator, failure_report):
    if failure_report.reason == "context_overflow":
        # オーケストレーター自身の学びとして保存
        learning = {
            "content": f"タスク「{failure_report.task_summary}」でコンパクション発生",
            "learnings": {
                "エージェント適性": f"このサイズのタスクは{failure_report.agent_type}には大きすぎた",
                "タスク結果評価": "タスク分割が不十分だった"
            }
        }
...
オーケストレーターは、エージェントがコンパクションを起こす前に介入する。
...
エージェントから状況報告を受け取る
{
    ...
    "context_usage": "60%",
    "current_work": "キー入力処理を実装中"
}
    ↓
判断:
├── 順調（進捗 > コンテキスト使用率）→ 継続
├── 危険（進捗 < コンテキスト使用率 * 0.7）→ 早期中断を指示
│   └── 「中断してください。ここまでの成果を報告してください」
└── 行き詰まり（進捗が停滞）→ 状況確認、必要ならユーザーに報告
```
...
```
例: コンテキスト60%使用、進捗30%
→ このペースでは100%完了前にコンパクション発生
→ 早期中断を指示

早期中断後:
├── エージェントは部分成果を報告して睡眠
├── オーケストレーターは残りを新しいタスクとして分割
└── 別のエージェントインスタンスに依頼
```
...
```
行き詰まりの検出:
├── エージェントから "blocked" ステータスを受け取る
├── 進捗確認で進捗が停滞している
└── 同じ作業に長時間かかっている

対応オプション:
1. タスク分割 → 詰まっている部分を別タスクに
2. 別アプローチ提案 → 別のエージェントまたは方法で
3. ユーザーに報告 → 技術的制約など判断が必要な場合

重要: 勝手に判断せず、判断が必要な場合はユーザーに報告
```
...
【問題】
IMEの変換候補ウィンドウ位置調整がAndroid OSの制限により困難

【試したこと】
- adjustResize: 一部機種で動作せず
- WindowInsets API: 不安定
...
```
完了判断:
├── 全サブタスクが完了（pending_subtasksが空）
├── ユーザーに確認を送信
└── ユーザーOK → 完了 / 追加要望あり → 継続

完了報告の形式:
├── 成果物: 何ができたか
├── 未対応事項: 対応できなかったこと（理由含む）
└── 改善案: 今後の発展可能性
```
...
【成果物】
- Android SSH/SFTPクライアント
- バッククォート入力可能な特殊キーパネル

【未対応事項】
- IME候補ウィンドウ位置調整（Android OS制限のため）

【改善案】
- 将来的にカスタムIME対応を検討
- 対応機種限定版の開発

確認をお願いします。追加で必要なことはありますか？」
```

#### オーケストレーターの睡眠トリガー

専門エージェントは「1タスク完了 = 睡眠」だが、オーケストレーターはユーザーとの対話を継続的に管理するため、睡眠タイミングが異なる。
...
1. コンテキスト70%到達
   → コンパクション回避のための強制睡眠
   → 進捗サマリを外部メモリに保存してセッション終了

2. アイドル状態が1時間継続
   → ユーザーからの指示なし AND エージェントへの指示中タスクなし
   → 待機時間を有効活用して減衰・整理
...
| | 専門エージェント | オーケストレーター |
|--|-----------------|-------------------|
| 睡眠トリガー | タスク完了 | 上記3条件のいずれか |
| 睡眠処理 | 同じ | 同じ |
| 減衰・アーカイブ | 同じ | 同じ |

睡眠処理の内容（減衰、アーカイブ、強制剪定）は完全に同じ。違いはトリガー条件のみ。
...
従来: 待機中は何もしない
    ↓
提案: 待機中に睡眠して記憶整理
    → 次のセッション開始時は「整理された状態」で再開
    → 直前の会話は外部メモリから検索して読み込み
```

**中間睡眠後の再開フロー**：

```
コンテキスト70%到達 または 1時間待機
    ↓
睡眠（学び保存、減衰、アーカイブ、剪定）
    ↓
セッション終了
...
これにより、どれだけ大きなタスクでもコンパクションによる情報損失を防ぎつつ、オーケストレーターの学びを永続化できる。

#### 進捗メタ情報の管理

中間睡眠では「進捗状態」を外部メモリに保存し、起動時に最優先で読み込む。これは通常の学びとは異なる特別な情報として扱う。
...
```json
{
  "id": "progress_current",
  "type": "progress_state",
  "content": "アプリケーション開発タスクの進捗",
  "progress": {
    "user_request_id": "req_001",
    "original_instruction": "アプリケーション開発して",
    "status": "in_progress",
    "completed_subtasks": [
      {"id": "subtask_01", "summary": "要件定義", "result": "完了"},
      {"id": "subtask_02", "summary": "基本設計", "result": "完了"}
    ],
    "current_subtask": {
      "id": "subtask_03",
      "summary": "認証機能の実装",
    ...
    "context_summary": "要件定義完了、基本設計完了、現在実装フェーズ。認証はJWT方式で実装予定。"
  },
...
def prepare_orchestrator_context(orchestrator):
    ...
    context = {
        "progress_state": progress,          # 最優先
        "recent_conversation": recent_history,  # 短期記憶
        "relevant_memories": relevant_memories   # 長期記憶からの検索結果
    }
    return context
```
...
進捗情報はタスク完了時に削除され、その内容から抽出された「学び」が通常の記憶として保存される。

#### 進捗状態の外部記録（ユーザ可視化用）

オーケストレーターは、タスクの指示または結果受領時に必ず進捗状態を外部ファイルに記録する。これは：
1. オーケストレーターが中間睡眠しても失われない情報
2. ユーザがいつでも確認できる進捗レポート
...
```json
{
  "user_request": {
    "id": "req_001",
    "original": "Android SSH/SFTPクライアント開発（日本語入力対応）",
    "clarified": "自分用、バッククォート・IME・特殊キー対応"
  },
  ...
  "task_tree": {
    "調査": {
      "status": "completed",
      "children": {
        "既存クライアント調査": {"status": "completed", "output": []},
        "IME API調査": {"status": "completed", "output": ["docs/ime_api_notes.md"]},
        "OSSライブラリ評価": {"status": "completed", "output": ["docs/library_comparison.md"]}
      }
    },
    "設計": {
      "status": "completed",
      "depends_on": ["調査"],
      "children": {
        "アーキテクチャ設計": {"status": "completed", "output": ["docs/architecture.md"]},
        "TerminalView設計": {"status": "completed", "output": []},
        "特殊キーパネル設計": {"status": "completed", "output": []}
      }
    },
    "実装": {
      "status": "in_progress",
      "depends_on": ["設計"],
      "children": {
        "基盤構築": {"status": "completed", "output": ["app/build.gradle"]},
        "SSH接続": {"status": "completed", "output": ["app/src/main/java/SSHClient.kt"]},
        "TerminalView": {
          "status": "in_progress",
          "children": {
            "基本レンダリング": {"status": "completed", "output": ["app/src/main/java/TerminalView.kt"]},
            "キー入力処理": {"status": "in_progress", "output": []},
            "特殊キーパネル": {"status": "pending", "depends_on": ["キー入力処理"], "output": []}
          }
        },
        "統合": {"status": "pending", "depends_on": ["SSH接続", "TerminalView"], "output": []},
        "SFTP": {"status": "pending", "depends_on": ["統合"], "output": []}
      }
    }
  },
  ...
  "history": [
    {
      ...
      "type": "user_decision",
      "issue": "SFTPディレクトリ一覧が遅い",
      "decision": "遅いまま実装続行",
      "options_presented": ["別ライブラリ", "SSHのみ", "遅いまま続行", "調査継続"]
    }
  ]
}
```
...
ユーザはこのファイルを直接見る（JSON/YAML）か、将来的にViewerを通じて進捗を確認できる。

#### 成果物の引き継ぎ（ハイブリッド方式）

専門エージェントは1タスク1セッションで睡眠するため、後続タスクのエージェントは前のエージェントの成果物を知らない。オーケストレーターがこの引き継ぎを管理する。

**基本方針**: ファイルパスを渡す + 必要に応じてヒントを追加

```python
# 単純なケース：ファイルパスのみ
task_instruction = {
    "task": "特殊キーパネル実装",
    "description": "Ctrl、Esc、バッククォートなどの特殊キーを入力するパネルを実装",
    "input_files": [
        "app/src/main/java/TerminalView.kt"  # 進捗状態から取得
    ]
}

# 複雑な統合タスク：ヒントを追加
task_instruction = {
    "task": "SSH/TerminalView統合",
    "description": "SSH接続とターミナル表示を統合する",
    "input_files": [
        "app/src/main/java/SSHClient.kt",
        "app/src/main/java/TerminalView.kt"
    ],
    "integration_hints": "SSHClient.getInputStream() を TerminalView.setInputStream() に接続"
}
```
...
この方式により：
1. 基本はシンプル（ファイルパスを渡すだけ）
2. 必要な場合のみオーケストレーターがヒントを追加
3. 過度な仕組みを作らない
4. エージェントはファイルを読んで理解する（人間の開発者と同じ）

### 3.4 外部メモリ構造（専門エージェント用）
...
#### 専門エージェントの重要な制約：中間睡眠なし

**専門エージェントは中間睡眠しない。オーケストレーターのみが中間睡眠を持つ。**

```
オーケストレーター:
├── 複数のサブタスクを管理
├── 中間睡眠あり（コンテキスト70%、アイドル1時間等）
├── 進捗状態を外部メモリに保存して再開可能
└── ユーザーとの対話を継続
...
**なぜ専門エージェントは中間睡眠しないのか**：

```
専門エージェントが中間睡眠した場合の問題:
...
├── 外部メモリから「続き」を読み込み
│   └── でも実装の細かい文脈は失われている
├── 「この変数は何のためだっけ」「なぜこの設計にしたんだっけ」
└── 一貫性崩壊、最初からやり直しに近くなる
```

コードや設計の「なぜこうしたか」という文脈は要約しにくく、中断→再開で「考えの流れ」が途切れる。だから専門エージェントは1タスク1セッションで完結させる。

#### コンパクション = タスク失敗

専門エージェントでコンパクションが発生した場合、それは**タスク失敗**として扱う。
...
1. 原因の判定
   ├── タスクが大きすぎた
   │   └── 報告に「作業量多い」「時間がかかる」が含まれる
   └── タスクが曖昧すぎた
       └── 報告に「仕様不明」「要件確認」「試行錯誤」が含まれる
...
この学習により、オーケストレーターは同様のタスクを次回から適切に分割できるようになる。
...
入力処理層は、ユーザー入力を受け取りオーケストレーターに渡す前処理を行う。軽量LLM（Haiku等）で実装し、以下の判断を行う：

```
入力処理層の判断フロー:

1. 論点数を検出
   - 10個未満 → そのままオーケストレーターへ
   - 10個以上 → 警告・オプション提示

2. 入力サイズを確認
   - 小さい → そのままオーケストレーターへ
   - 大きい → 概要生成してオーケストレーターへ
```

**重要な設計判断**: 入力処理層は「解釈」や「曖昧さの解消」を行わない。曖昧な指示もそのままオーケストレーターに渡す。曖昧さの解消はオーケストレーターがユーザーへの質問で行う。
...
エージェントは外部メモリと睡眠フェーズで「記憶」を管理するが、タスク実行には「行動能力」も必要。これはツールとして定義する。

```
エージェントの構成:
├── 役割定義（role）
├── 観点定義（perspectives）
├── 外部メモリ（external_memory_ref）
...
ツールはJSON Schemaベースで定義する。Claude APIやMCP（Model Context Protocol）と互換性のある形式を採用。

```json
{
  "agent_id": "research_agent_01",
  "role": "調査エージェント",
  "perspectives": ["情報源", "信頼性", "関連性", "網羅性", "最新性"],
  "tools": [
    {
      "name": "web_search",
    ...
    {
      "name": "web_fetch",
      "description": "指定したURLのコンテンツを取得する",
      "input_schema": {
        "type": "object",
        "properties": {
          "url": {
...
```
1. LLMがツールの必要性を判断
   ├── タスクの内容とツールの description を照合
   └── 必要なら tool_use ブロックを出力

2. ホスト側がツールを実行
   ├── tool_use から name と input を抽出
   ├── 実際のツール関数を呼び出し
...
| 役割 | ツール | 用途 |
|------|--------|------|
| 調査エージェント | web_search, web_fetch | 情報収集 |
| 実装エージェント | read_file, write_file, edit_file, run_code | コード作成・実行 |
| テストエージェント | run_tests, analyze_coverage | テスト実行・分析 |
| 統合エージェント | api_call, database_query | 外部システム連携 |
| オーケストレーター | ask_user, delegate_task | ユーザー対話・タスク委譲 |

#### ツール使用の学習

ツールの使用経験も外部メモリに保存され、学習の対象となる。

```json
{
  "id": "mem_701",
  "content": "GitHubのAPIレート制限に遭遇",
  "learnings": {
    "情報源": "GitHub APIは1時間に60リクエストまで（未認証時）",
    "信頼性": "レート制限エラーは429ステータスで返る",
    "関連性": "大量のリポジトリ調査時は認証トークンが必要"
  },
  "tags": ["GitHub", "API", "レート制限", "tool:api_call"],
  "strength": 1.0
}
```

次回同様のタスクで「GitHub API」を検索すると、この学びがヒットし、事前にレート制限を考慮した計画を立てられる。
...
経路2: 実践学習（Learning by Doing）
├── 教材がない場合、または新しい分野
├── 実際のタスク実行を通じて学習
└── LLMの事前学習知識を活用
```

**LLMは既に「専門家以上の知識」を持っている**。教育プロセスは「何も知らない状態からの学習」ではなく、「どの知識を優先するか」の重み付けである。

### 4.2 実践学習：教材なしでの専門性形成

教科書がない場合、エージェントは実践を通じて学ぶ。LLMの事前学習知識がベースとなり、実際に使われた知識が強化される。
...
タスク1: 「サプライヤーを評価して」
├── LLMが事前学習知識で回答を生成
├── 使用した観点（コスト、品質等）を学びとして抽出
├── 睡眠 → 外部メモリに保存
└── ユーザーフィードバック → 成功/失敗を記憶
...
結果: 「このエージェントはコスト重視の評価が得意」という個性が形成
```
...
| 観点 | 教科書学習 | 実践学習 |
|------|-----------|---------|
| 初期知識 | 教材から体系的に | LLMの事前学習知識 |
| 学習効率 | 高い（構造化された教材） | 低い（試行錯誤が必要） |
| 適用範囲 | 教材がある分野 | 任意の分野 |
| 個性の形成 | 教材に依存 | 経験に依存 |
...
| オーケストレーター | 専門エージェントと同じ仕組みで動くエージェント。役割と観点が異なるだけ |
| 統一設計 | オーケストレーターと専門エージェントが同じメカニズム（外部メモリ、強度管理、減衰等）を共有する設計原則 |
| 暗黙的フィードバック | 明示的な発言なしに検出するユーザーシグナル（例：結果をそのまま使用＝成功） |
| アイドル睡眠 | ユーザー指示やエージェントへの指示中タスクがない状態が一定時間継続した際に実行される睡眠 |
| 中間睡眠 | コンテキスト70%到達などで、タスク完了前に実行される睡眠。進捗を外部メモリに保存して再開可能 |
| ツール（tool） | エージェントの行動能力を定義するJSON Schema形式の関数定義。Claude APIやMCPと互換 |
| 進捗状態（progress_state） | 中間睡眠時に保存されるタスクの進捗情報。起動時に最優先で読み込まれる |
| 実践学習 | 教材なしで実際のタスク実行を通じて専門性を形成する学習方式 |
| 入力処理層 | ユーザー入力を前処理してオーケストレーターに渡すレイヤー。軽量LLMで実装 |
| 早期中断 | エージェントがコンパクションを起こす前にオーケストレーターが介入してタスクを中断させること |
| 進捗監視 | オーケストレーターが定期的にエージェントの進捗とコンテキスト使用率を確認する仕組み |
| コンパクション失敗 | 専門エージェントでコンパクションが発生した場合のタスク失敗。オーケストレーターのタスク分割不足が原因 |
| タスクサイズ | タスクの大きさ。専門エージェントのコンテキストウィンドウ内で完了できるサイズに分割する必要がある |
| 進捗状態ファイル | オーケストレーターがタスク指示/結果受領時に更新する外部ファイル。ユーザの進捗確認と中間睡眠からの復帰に使用 |
| タスクツリー（task_tree） | タスク間の依存関係と完了状況を表すツリー構造。進捗状態ファイルに記録される |
| 成果物引き継ぎ | 前のエージェントの成果物を後続エージェントに伝える仕組み。ファイルパス＋必要に応じてヒントを渡すハイブリッド方式 |
